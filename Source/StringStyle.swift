...
extension StringStyle {
  ...
  fileprivate func addCommandSeperators(_ string: String) -> String {
    var rangeWithInset = (string.characters.index(after: string.startIndex) ..< string.index(before: string.endIndex))
    let newString = string.replacingOccurrences(of: startOfCode, with: ";", options: .literal, range: rangeWithInset)
    
    rangeWithInset = (newString.index(after: newString.startIndex) ..< newString.characters.index(before: newString.endIndex))
